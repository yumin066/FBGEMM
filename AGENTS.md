...
- Phase 36：目标是统一并重构 SM120 FP8 WS。所有 FP8 WS 配置都应走 persistent kernel；pure causal 等三角工作量不均衡配置使用 paired persistent，其余配置使用 non-paired static grid-stride persistent。重构要求包括：精简冗长注释、让代码结构直接体现 warp-specialized 架构和 Q/K/V/RAB/O handoff 流程、把重复 load warp / mbarrier / TMA stage 逻辑抽成 `__device__ __forceinline__` helper、减少 `hstu_compute_attn_1rowblock_sm120_fp8_ws` 内部 lambda 嵌套。任何重构都不能造成性能 regression，必须用 correctness、SASS spill 检查和锁频 benchmark 闭环。
- Phase 35：已完成 SM120 BF16 CuTe DSL 支持面向 BF16 CuTe C++ 路径的补齐。C++ kernel/dispatch 覆盖 D32/D64/D128/D256 × full/causal/local/context/target/arbitrary × none/RAB/DRAB；BF16 paged KV 与 DSL 保持同一口径，通过 wrapper-side dense materialization 后调用现有 C++ BF16 kernel，不是原生 paged BF16 kernel。最终 correctness 日志：`1test_results/788_phase35_bf16_cpp_full_hstu_test_after_arbitrary_fix.log`、`1test_results/789_phase35_bf16_cpp_sweep_after_arbitrary_fix.log`、`1test_results/790_phase35_bf16_cpp_hstu8_examples_after_arbitrary_fix.log` / `791`。锁频 benchmark：`2benchmark_results/646_c36869fd_gpu2407MHz_phase35_bf16_cpp_dsl_dense_paged_all_config.log`。
- Phase 34：持续分析 FP8 WS benchmark 并对低 TFLOPS case 做 profile/优化。第一轮聚焦 `bs=1/8,seq=2048,h=4/16,d=256` 的 RAB/DRAB 与 pure causal。已保留的小优化是：`D=256 pure causal RAB/DRAB` 且 `params.b * params.h <= 4` 时，aligned RAB tile 不走 K-warp RAB TMA/SMEM，而回退到 direct-global RAB add，避免小工作量下 RAB TMA/mbarrier 固定开销。`local/context/target/arbitrary` 和大 batch/head causal 仍走 RAB TMA/SMEM。验证日志：`1test_results/652_phase34_d256_small_causal_rab_direct_final_hstu_test.log` 为 `3 passed, 1 skipped`，`1test_results/653_phase34_d256_small_causal_rab_direct_final_sweep.log` 通过，`1test_results/655_phase34_d256_small_causal_rab_direct_final_examples_retry.log` 为 `300/300 passed`。锁频 benchmark：`2benchmark_results/641_gpu2407MHz_phase34_d256_s2048_small_causal_rab_direct_focus.log` 和恢复确认 `643`。
...
- Phase 34 后，`D=256 pure causal RAB/DRAB` 在小工作量 `params.b * params.h <= 4` 时也使用 direct-global RAB add 作为性能启发式；这只针对 pure causal，小工作量 `local` 实测回退因此不启用。
