...
Phase 36 当前状态：FP8 WS scheduler 第一刀已完成并保留。所有 SM120 FP8 WS 配置都走 persistent kernel；pure causal/部分 paged target 使用 paired persistent，其余 full/local/context/target/arbitrary/RAB/DRAB 使用 non-paired static grid-stride persistent。load/math 分支已共用 `hstu_ws_run_static_schedule` inline helper，避免重复 tile decode；RAB full-persistent 和 paired-persistent 路径现在都在 tile 间保留 CTA sync，防止 single-stage RAB SMEM 被下一 tile 覆盖。第一批 reviewability 重构已把 FP8 WS launch helper 从 `hstu_fwd_kernel.h` 拆到 `hstu_fwd_kernel_fp8_ws_launch.h`，并抽取了若干 mbarrier/stage/O-store helper；FP8 WS 主循环里的 `debug_gemm1_only` 分支已删除。当前最终 no-debug correctness 通过：5x matrix `876`、完整 `hstu_test.py` retry `878`、sweep `879`、examples `880`/`882`。D128 causal+RAB focused benchmark `665` 相对 `663` 有小幅回退：non-paged FP8 geomean `-2.71%`、paged geomean `-0.62%`；这是 correctness sync 的代价，后续应通过 RAB double buffer 或更细 handoff 优化，而不是恢复 debug 分支。
