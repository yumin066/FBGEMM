...
- 残余风险：D256 causal/context/target 复杂 mask 路径仍有少量 stack/local spill，但 profile 显示它不是当前首要瓶颈。下一步性能优化应优先处理 RAB/paged target 的 memory path 和 No Eligible，而不是直接追着 residual spill 做局部重写。
