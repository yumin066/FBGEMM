...
Phase 40 当前目标：全量对比 SM120 FP8 kernel 与 H100 FP8/BF16 kernel。执行方式：Codex 在 SM120 host 上运行；SM120 侧通过 `./docker_computelab.sh` 进入容器并运行全量 benchmark；H100 侧先用 `squeue -u minyu` 找到 Slurm 分配节点，再 `ssh <h100-node>`，进入共享仓库后同样通过 `./docker_computelab.sh` 进入容器运行 benchmark。H100 编译产物必须隔离到 H100 专用目录，优先使用 `.build/sm9x/` 或等价 `PYTHONUSERBASE`，不能覆盖 SM120 的 `.build/sm12x/` 或源树已有 `.so`。结果统一放到 `6cross_bench_results/`，并生成共同 case 的 SM120 FP8 vs H100 FP8/BF16 对比表。

Phase 40 首轮已完成：`bench_cross_arch.sh` 已支持 per-arch isolated build prefix；SM120 使用 `.build/sm12x/`，H100 使用 `.build/sm9x/`。`bench_hstu_cross_arch.py` 已补齐 H100/SM90 FP8 `quant_mode=2` kernel-only 计时列，并支持 `--headdims`。SM120 在本机容器内锁频 2407MHz 完成 `216` 个 common case，日志/CSV 为 `6cross_bench_results/rtx_pro_6000_blackwell_server_edition_c48f38e4_20260512_0209.log` 和 `.csv`；H100 节点为 `ipp2-0160`，容器内锁频 1980MHz 完成 `216` 个 common case，日志/CSV 为 `6cross_bench_results/h100_80gb_hbm3_c48f38e4_20260512_0249.log` 和 `.csv`。汇总文件为 `6cross_bench_results/compare_sm120_fp8_vs_h100_fp8_bf16_c48f38e4_20260512_0249.md`。共同矩阵：`bs={1,4,8} × seq={512,1024,2048,4096} × h={4,8,16} × d={64,128,256} × mask={full,causal}`；两边 BF16/FP8 均无 error。总体 TFLOPS geomean：SM120 FP8 / H100 FP8 为 `1.443x`，SM120 FP8 / H100 BF16 为 `1.229x`；H100 FP8 / H100 BF16 为 `0.852x`。D64/D128 是 SM120 FP8 强项，分别为 H100 BF16 的 `1.393x/1.293x`；D256 是弱项，geomean 仅 `1.032x`、median `0.966x`，慢点集中在 `bs=1, seq>=2048, d=256`。

Phase 38 当前目标：在冻结 Phase 37 recsys baseline 的前提下优化 SM120 FP8 WS RAB/DRAB。性能基线固定为 recsys-examples exact target case 的锁频日志：non-paged `2benchmark_results/714_recsys_phase37_final_candidate_nonpaged_rerun_gpu2430MHz.log`，paged `2benchmark_results/713_recsys_phase37_final_candidate_paged_gpu2430MHz.log`。后续所有优化必须保证 no-RAB recsys exact case 不低于该基线，并继续通过 `hstu_test.py`、`sweep_accuracy.py`、`run_hstu8_examples.sh` 三件套。

Phase 38 实施计划：第一步冻结 Phase 37 recsys no-RAB baseline，并用 recsys-examples exact target case 直接复现 `bs=1/8, dim=256, h=4/16, history_length=2048, new_history_length=2048, target=256` 的 RAB/DRAB 性能；本项目主 benchmark 暂不纳入 h=16 recsys exact case。第二步优化 RAB/DRAB 数据路径：优先保留 `h_rab == 1` 的 head-shared scheduler 和 BF16 RAB TMA/SMEM 稳定路径。第三步继续专项处理低效 case：non-paged RAB/DRAB 关注 h=16、bs=8 的带宽复用，paged RAB/DRAB 关注 page-id/control-flow 与 RAB TMA 固定开销，D256 target/context/local 的残余 stack 只做低风险 compile-time 常量化。暂不把 BF16 CuTe DSL 作为主线，除非目标切回 DSL 研究。

Phase 38 当前进展：recsys-examples benchmark 已增加 RAB/DRAB 复现入口（`--num-heads`、`--bias-config none/rab/drab`、`--rab-heads per-head/shared`、`--max-seqlen-mode active`），默认行为不变。锁频 exact baseline：non-paged per-head `2benchmark_results/717_phase38_recsys_exact_h4_nonpaged_bias_baseline_gpu2430MHz.log`、`719_phase38_recsys_exact_h16_nonpaged_bias_baseline_gpu2430MHz.log`；non-paged shared `718`、`720`；paged per-head `726_phase38_recsys_exact_paged_h4_h16_bias_baseline_gpu2430MHz.log`；paged shared `727_phase38_recsys_exact_paged_h4_h16_bias_shared_gpu2430MHz.log`。代表 baseline：h=4 non-paged no-RAB/RAB/DRAB bs1 `0.088/0.115/0.115ms`，bs8 `0.217/0.291/0.290ms`；h=16 non-paged no-RAB/RAB/DRAB bs1 `0.135/0.175/0.175ms`，bs8 `0.726/0.922/0.922ms`。`h_rab=1` 对 h=16 bs1 最有效，RAB 降到约 `0.123ms`；对 bs8 有小到中等收益。

Phase 38 已否决实验：FP8 RAB + scalar descale 不纳入 SM120 支持面，代码已回退；SM120 RAB/DRAB 与 Ampere/Hopper 保持 BF16 RAB 口径。direct-global FP8 RAB smoke 日志 `2benchmark_results/729_phase38_recsys_h4_nonpaged_fp8_rab_smoke_gpu2430MHz.log` 中 h=4 bs1 non-paged RAB 约 `0.199ms`，显著慢于 BF16 RAB。FP8 RAB TMA/SMEM 在 non-paged 上仅有小幅收益，但 paged `733` 相比 BF16 RAB `736` 慢约 `4%~10%`，且会扩大 API/dispatch/测试矩阵，不符合当前目标。

Phase 38 最新 benchmark 闭环：锁频 2430MHz 后完成当前 repo 全量 kernel-only benchmark `2benchmark_results/737_0808f1d5_gpu2430MHz_phase38_full_repo_kernel.log`，共 2592 行，`Unsupported selected cases: none`，无 `ERROR`/`Traceback`/`CUDA error`。recsys-examples 按 05-08 同口径完成 no-RAB dim256 kernel-only 对比：non-paged `2benchmark_results/738_0808f1d5_gpu2430MHz_recsys_0508_nonpaged_compare.log`，paged `2benchmark_results/739_0808f1d5_gpu2430MHz_recsys_0508_paged_compare.log`。相对 05-08 baseline `hstu_kernel_sm120_fp8_dim256_20260508.log`，non-paged 84 个共同 case geomean `0.9809x`、median `0.9834x`、max `0.9985x`，无 `>0.5%` 慢点；paged 84 个共同 case geomean `0.9998x`、median `1.0000x`、max `1.0032x`，无 `>0.5%` 慢点。指定 recsys case `history=2048,new_history=2048,target=256`：non-paged bs1/bs8 为 `0.9860x/0.9817x`，paged bs1/bs8 为 `1.0001x/0.9965x`，未观察到相对 05-08 的性能回退。

Phase 39 当前实验目标：优化 D256 no-RAB 的 FP8 WS aliased O store。当前 D256 O 需要 64KB BF16 SMEM，直接覆盖两段 K/V double buffer，因此 math epilogue 前有 `bar.sync 1,256`，K/V load warp 也会被 O-store ownership 阻塞。最终采用的方案不是 split-O：D256 O 始终保持一整块连续 SMEM，与 Q persist 和逻辑 KV stage0 共享；逻辑 KV stage0 映射到靠近 Q persist 的上半 KV 区域，逻辑 KV stage1 映射到下半 KV 区域。每个 M tile 根据 N tile 个数选择初始 K/V stage，偶数从 stage0 开始、奇数从 stage1 开始，保证最后一个 N tile 落在 stage1，O store 始终覆盖 Q + stage0 连续区域。dim128 不进入该路径。验收：编译后跑 `hstu_test.py`、`sweep_accuracy.py`、`run_hstu8_examples.sh`；性能必须锁频跑当前 repo 全量 benchmark 和 recsys-examples benchmark。

Phase 39 当前进展：D256 no-RAB Q+KV stage0 连续 O 方案已实现，改动范围为 FP8 WS traits、TMA launch params 和 WS kernel 内 O-store/Q-empty/KV-stage handoff；已移除 split-O 的 half TMA descriptor 和半块 O 管理。Correctness 已通过：`1test_results/1019_qkv0_o_hstu_test.log` 为 `4 passed`，`1test_results/1020_qkv0_o_sweep.log` 通过，`1test_results/1021_qkv0_o_hstu8_examples.log` / 内部 `1022_hstu8_examples_qm2.log` 为 `300/300 passed`。SASS 代表实例 `4sass_dump_ws/hstu_fwd_kernel_sm120_fp8_ws_tma_I256_full_qkv0_o.sass` 无 `LDL/STL`、无 `LOCAL/STACK`，日志为 `1test_results/1022_qkv0_o_i256_full_sass.log`。锁频当前 repo kernel-only 全量结果为 `2benchmark_results/1023_c48f38e4_gpu2407MHz_qkv0_o_full_repo_kernel.log`；相对 split 前 `2benchmark_results/1014_3e9b2cf1_gpu2407MHz_trywait_kernel_only.log`，全量 common rows FP8 latency geomean `1.0075x`、paged `1.0069x`，D256 子集 FP8 `1.0040x`、paged `1.0062x`。recsys-examples exact target case 日志为 `benchmark_results/sm120_fp8_qkv0_o_kernel_only_dim256_h{4,16}_{nonpaged,paged}_20260511.log`；相对 split 前 `sm120_fp8_d256_swizzled_o_alias_*`，8 个 case geomean `0.9905x`，即 latency 降低约 `0.95%`；相对 split-O 版本 geomean `1.0237x`，即放弃了 split-O 的部分 recsys 收益，但代码和同步模型更简单，符合当前连续 O 管理要求。

Phase 37 当前状态：recsys-examples 中 SM120 FP8 kernel-only 性能回退已定位并处理。只比较两个文件都有的 kernel-only 口径，full-flow/E2E 不参与判断。05-09 汇总使用 fbgemm-hstu HEAD `5eb8ef38`，原始日志为 recsys-examples 的 `benchmark_results/sm120_fp8_kernel_full_dim256_20260509.log`；05-08 baseline 为 `benchmark_results/hstu_kernel_sm120_fp8_dim256_20260508.log` / `comparison_dim256_20260508.md`。独立 worktree bisect 结果：`f8b71818 All configs for bf16 cute c++` 单点 `(4,1024,512,256)` 为 `0.059902ms`，基本等同 05-08 的 `0.058192ms`；`030b04e1 All FP8 use persistent` 为 `0.110438ms`，复现 05-09 的 `0.106629ms` 级别回退。首个回退 commit 是 `030b04e1`，直接原因是把 non-paged target/local/context/arbitrary 等复杂 mask 路径从原 3D grid 切到 non-paired persistent。

Phase 37 当前保留策略：`hstu_fwd_kernel_fp8_ws.h` 恢复到 `f8b71818` 的 WS kernel body，并只增加 D256 non-paged pure target no-RAB 的 `target_group_size == 1` 编译期 specialization；该 specialization 资源为 `REG:168 STACK:8 LOCAL:0`，默认 target 分支为 `REG:168 STACK:16 LOCAL:0`。`hstu_fwd_kernel_launch.h` 将 persistent 收窄回 pure full 和已验证的 paired causal/paged-target，复杂 mask 路径回到 3D grid。

Phase 37 验证：锁频 recsys 对 05-08 baseline，non-paged `2benchmark_results/714_recsys_phase37_final_candidate_nonpaged_rerun_gpu2430MHz.log` 为 84 case geomean `0.9776x`、median `0.9794x`、max `1.0009x`、无 `>0.5%` 慢点；paged `2benchmark_results/713_recsys_phase37_final_candidate_paged_gpu2430MHz.log` 为 84 case geomean `0.9993x`、median `0.9999x`、max `1.0004x`、无 `>0.5%` 慢点。全量脚本残留的 `bs=1,seq=1024` 亚 0.1% 慢点经单 case 重复 `2benchmark_results/716_recsys_phase37_final_candidate_nonpaged_bs1_seq1024_repeat_gpu2430MHz.log` 验证为测量波动，同 case median 不慢于 05-08。Correctness 已通过：`1test_results/948_phase37_recsys_fix_hstu_test.log` 为 `4 passed`，`1test_results/949_phase37_recsys_fix_sweep.log` 通过，`1test_results/950_hstu8_examples_qm2.log` 为 `300/300 passed`。不保留的实验：target non-paged paired persistent、paged target specialization、D256 paged small-history cp.async threshold，以及恢复 HEAD/5eb8 WS body；这些实验会造成 selected 或全量性能回退。

Phase 36 当前状态：FP8 WS 重构中的 all-persistent scheduler 方案已被 Phase 37 性能回归分析否决，不作为当前默认路径。保留的结论是：pure full 和已验证的 paired causal/paged-target 可以 persistent；non-paged target/local/context/arbitrary 等复杂 mask 路径当前应保持原 3D grid，除非后续有 profile 证据证明新的 persistent 版本逐 case 不回退。Phase 36 的 reviewability 重构仍作为后续目标，但必须以 Phase 37 的调度边界为约束，不能重新引入 all-persistent。
...
- SASS：新增 persistent 配置代表实例无新增 local spill；历史已知 D256 causal/context/target 与部分 RAB 残余 spill 不得恶化。
...
- Rejected：全维度统一 `setmaxnreg` 为 load warp `40`、math warp `232` 不保留。`hstu_test.py::HSTU8Test::test_sm120_fp8_blockscale_matrix` 在 D128 causal+RAB/DRAB、seq=256 附近稳定出现 max-error 放大；日志见 `1test_results/836_sm120_matrix_setmaxnreg_40_232_repro.log`、`837_sm120_matrix_setmaxnreg_40_232_rab_syncwarp.log`、`838_sm120_matrix_setmaxnreg_40_232_mbar_memory.log`。D128 causal+RAB SASS `4sass_dump_ws/hstu_fwd_kernel_sm120_fp8_ws_tma_I128_causal_rab_setmaxnreg40_232.sass` 显示 load 分支在 `USETMAXREG.DEALLOC 0x28` 后最大只用到 `R37`，整 kernel `REG:168 STACK:0 LOCAL:0`，因此不是简单 local spill 或 load 分支越界访问高号寄存器。后续需要专项分析 all-40/232 为何会触发 RAB 路径数值错误；当前保留策略是 `kHeadDim > 128` 使用 load/math `40/232`，`kHeadDim <= 128` 继续使用 `56/224`。
- 最新 SASS/benchmark 复核：D256 代表 SASS 已输出到 `4sass_dump_ws/422_phase36_cond_I256_*.sass`。D256 `full`/`full+RAB`/`causal+RAB` 为 `STACK:0` 且无 `LDL/STL`；D256 no-RAB causal 为 `STACK:8`、context 为 `STACK:16`、target 为 `STACK:24`，D256 context+RAB 为 `STACK:8`、target+RAB 为 `STACK:24`。这些残余 spill 与当前 D256 复杂 mask 路径相关，不是 all-40/232 D128 correctness 问题。
- 最新锁频复跑：`2benchmark_results/651_d605d70d_gpu2407MHz_phase36_setmaxnreg_conditional_full.log` 的 kernel-only 表完整；e2e 表因与本次 kernel 寄存器预算验证无关而中断。相对 `649`，non-paged FP8 common geomean `+1.51%`，paged common geomean `+0.98%`；D256 non-paged FP8 geomean `+1.32%`，D256 paged geomean `-0.08%`。关注形状 `bs=8,seq=2048,h=16,d=256` 中，non-paged RAB/DRAB 多数提升 `+0.3%~+3.5%`，pure no-RAB full/causal/context 为 `-0.3%/-1.8%/-0.8%` 小幅波动，paged RAB/DRAB 多数在 `-1.9%~+2.5%` 区间。
- 最新 correctness 闭环：`1test_results/842_phase36_setmaxnreg_conditional_sweep.log` 通过；`1test_results/843_phase36_setmaxnreg_conditional_hstu8_examples.log` 通过，脚本内部日志 `1test_results/847_hstu8_examples_qm2.log` 为 `300/300 passed`。结合此前 `1test_results/840_hstu_test_setmaxnreg_conditional.log`，当前版本三项 correctness 入口均已覆盖。
- 最新 reviewability 重构：FP8 WS launch/host helper 已从 `hstu_fwd_kernel.h` 拆到 `fbgemm_gpu/experimental/hstu/src/hstu_blackwell_sm120/hstu_fwd_kernel_fp8_ws_launch.h`；FP8 WS kernel 内部已抽取 stage mbarrier、empty wait、K/V stage advance、O `stmatrix` store 和 OOB zero helper。该批改动目标是降低 review 负担，语义保持不变。
- 数值问题定位：初始删除 FP8 WS 主循环内 `params.debug_gemm1_only` 分支后，`hstu_test.py` 在 D128 causal+RAB/DRAB 附近失败，日志 `1test_results/854_phase36_refactor_hstu_test.log` 为 `2 failed, 2 passed`，代表误差为 HSTU max diff `0.0917` vs PyTorch FP8 max diff `0.0217`，超过 `2x` 容差。恢复该分支后单次 correctness 通过（`855`、`856`、`857`、`858`/`865`），但后续重复 matrix 暴露了 flaky failure：`1test_results/869_phase36_refactor_sm120_matrix_flake_5x.log` 前 3 次通过、第 4 次失败，失败点为 D128 seq=99 causal+RAB，max diff `0.374`。进一步实验表明，只给 mbarrier wait/arrive 加 `"memory"` clobber 不能解决，`1test_results/866_phase36_mbar_memory_no_debug_sm120_matrix.log` 仍失败。最终根因是 paired persistent + RAB single-stage SMEM 缺少 tile 间 CTA rendezvous；full-persistent RAB 已有 sync，但 paired-persistent RAB 没有。修复为 `Has_rab` 时 paired scheduler 在 pair slot / grid-stride pair 之间也执行 `__syncthreads()`。
- 最终 no-debug correctness：paired RAB sync 后，删除 `debug_gemm1_only` 分支仍可稳定通过 targeted matrix 5 连跑，日志 `1test_results/876_phase36_paired_rab_sync_no_debug_sm120_matrix_5x.log`。完整 `hstu_test.py` 第一轮 `877` 在 BF16/HSTU16 处出现一次 CUDA illegal-memory flake 并污染后续项，重跑 `1test_results/878_phase36_paired_rab_sync_no_debug_hstu_test_retry.log` 为 `4 passed`；`sweep_accuracy.py` `1test_results/879_phase36_paired_rab_sync_no_debug_sweep.log` 通过；`run_hstu8_examples.sh` wrapper 日志 `1test_results/880_phase36_paired_rab_sync_no_debug_hstu8_examples.log` / 内部 `1test_results/882_hstu8_examples_qm2.log` 为 `300/300 passed`。
- 最新 resource dump：`1test_results/881_phase36_paired_rab_sync_no_debug_resource.log` 扫描到 FP8 WS kernels `120` 个，`LOCAL:0` 全部为 0；`STACK` 非零实例 `21` 个，分布为 `8:4, 16:2, 24:9, 32:3, 40:1, 64:1, 80:1`。相比 debug-branch 版本 `859` 的非零 stack `14` 个有所增加，但没有 local spill。
- 最新锁频 focused benchmark：`2benchmark_results/665_gpu2407MHz_phase36_paired_rab_sync_no_debug_d128_causal_rab_kernel.log`。相对 `2benchmark_results/663_gpu2407MHz_phase36_refactor_debug_restore_kernel.log` 的 D128 causal+RAB/DRAB common case，non-paged FP8 geomean `-2.71%`，paged geomean `-0.62%`；代表形状 `bs=8,seq=2048,h=16,d=128 causal+RAB` 为 FP8 `227.4 -> 219.3 TFLOPS`，paged `221.4 -> 217.4 TFLOPS`。该回退是正确性 sync 的代价，后续优化目标是恢复 tile overlap，例如 RAB double buffer。
- 最新 resource dump：`1test_results/859_phase36_refactor_debug_restore_resource.log` 扫描到 FP8 WS kernels `120` 个，`LOCAL:0` 全部为 0；`STACK` 分布为 `0:106, 8:9, 16:1, 24:3, 32:1`。这与已知 D256/复杂 mask 残余 stack 风险一致，没有新增 local spill。
- 最新锁频 kernel-only benchmark：`2benchmark_results/663_gpu2407MHz_phase36_refactor_debug_restore_kernel.log`。相对 `2benchmark_results/651_d605d70d_gpu2407MHz_phase36_setmaxnreg_conditional_full.log` 的 kernel-only common case，non-paged FP8 geomean `+1.77%`，paged geomean `+1.35%`；`seq>=2048` 分别为 `+0.46%` / `+0.24%`。D128 causal+RAB/DRAB common case分别为 non-paged `+1.62%`、paged `+1.74%`。最差 common case 主要是小形状量化噪声或个别 long-seq RAB 波动，未观察到系统性回退。
- 最新函数抽取重构全量 benchmark：`2benchmark_results/671_179816cd_gpu2407MHz_phase36_refactor_final3_full_kernel.log`。该日志只评估当前已完成的 helper 抽取/热路径恢复，不包含后续 Q/K/V/O role 外提。相对 `663` 的 2592 个 common case，FP8 geomean `+1.03%`、paged geomean `+1.35%`；`seq>=2048` 为 FP8 `+0.20%`、paged `+0.55%`。D128/D256 的 `seq>=2048` FP8 分别为 `-1.15%/-1.20%`，属于局部小幅波动；未观察到系统性 regression，因此当前函数抽取重构保留。
- Rejected：Q/K/V/O role 外提到 persistent schedule 外层不保留。该版本编译通过，correctness 也通过（`1test_results/909_sm120_fp8_ws_role_outer_hstu_test.log`、`910_sm120_fp8_ws_role_outer_sweep.log`、`911/912` examples），但 resource/SASS 出现实质回退：`1test_results/913_sm120_fp8_ws_role_outer_resource.log` 中一个 D256 paged complex specialization 达到 `STACK:192`，`4sass_dump_ws/hstu_fwd_kernel_sm120_fp8_ws_role_outer_stack192.sass` 出现大量 `STL/LDL`。原因判断是同一 kernel 内实例化四套 role-specific tile body 增大代码生成和寄存器/stack 压力；相比之下，当前 tile 内 role bool 判断成本小于 spill 成本。源码和安装版本已恢复到 tile 内 role 判断。恢复后 resource 为 `1test_results/914_sm120_fp8_ws_role_outer_rejected_resource_after_revert.log`，120 个 FP8 WS kernel 均 `LOCAL:0`，无 `STACK:192`；correctness 恢复验证通过 `1test_results/915_sm120_fp8_ws_role_revert_hstu_test.log`、`916`、`917/918`。
- 最新 D256 profile：锁频 NCU 产物为 `3profile_results/652_phase36_d256_s2048_h16_fp8_causal_none_ncu.*`、`653_*context_none*`、`654_*target_none*`、`655_*target_rab*`、`656_*paged_target_rab*`。`bs=8,seq=2048,h=16,d=256` 下，target+RAB 相比 target 的 duration `1.97ms -> 2.46ms`，local spill requests `1.59M -> 1.39M` 反而下降，但 DRAM throughput `20.2% -> 60.6%`、L2 hit `89.9% -> 66.2%`，说明 RAB 回退主要来自 RAB memory traffic/L2 miss，而不是 spill。paged target+RAB 为 `0` local spill requests，但 duration `3.48ms`、No Eligible `68.1%`、Issue Slots Busy `28.6%`，说明 paged target+RAB 的主要问题是 page/RAB memory path 和 eligible warp 不足。
- Rejected：target+RAB sparse 区域改 direct-global RAB add 不保留。实验让 target-target N block 跳过 RAB TMA/SMEM，并在 math warp mask 后只读未 mask 的 RAB 元素；correctness 通过（`1test_results/844_target_rab_sparse_hstu_test.log`、`845_target_rab_sparse_sweep.log`、`846_target_rab_sparse_hstu8_examples.log`），但锁频 focused benchmark `2benchmark_results/657_gpu2407MHz_target_rab_sparse_focus.log` 显示 `bs=8,seq=2048,h=16,d=256 target+RAB` non-paged 从 `651` 的 `406.6 TFLOPS` 降到 `331.8 TFLOPS`，paged 基本不变，因此回退。
- Rejected：只把 target+RAB 的 RAB add 移到 mask 后并启用 skip-masked 也不保留。`2benchmark_results/658_gpu2407MHz_target_rab_skipmasked_only_focus.log` 中同一 non-paged target+RAB 降到 `325.9 TFLOPS`。说明当前 target 路径把 RAB wait/add 推迟到 mask 后会破坏流水/调度，即使仍保留 RAB TMA 也会回退。
- Rejected：让 D256 paged causal/target+RAB 走 paired persistent 不保留。单点 `bs=8,seq=2048,h=16,d=256` 有收益（target+RAB paged `307.0 -> 316.7 TFLOPS`，causal+RAB paged `285.4 -> 300.1 TFLOPS`；日志 `660`、`661`），但 D256 causal/target matrix `2benchmark_results/662_gpu2407MHz_paged_rab_paired_d256_causal_target_matrix.log` 相对 `651` 的 paged geomean 为 causal `-12.8%`、target `-18.0%`，且用户重点 `h=4` target 多数回退。SASS/resource 日志：`1test_results/849_paged_rab_paired_resource.log`，causal+RAB paired 为 `REG:168 STACK:0 LOCAL:0` 无 `LDL/STL`，target+RAB paired 为 `REG:168 STACK:8 LOCAL:0` 且有 2 对 `STL/LDL`；性能不满足保留条件，源码和安装版本已恢复。
- 残余风险：D256 causal/context/target 复杂 mask 路径仍有少量 stack/local spill，但 profile 显示它不是当前首要瓶颈。后续性能优化应优先处理 RAB/paged target 的 memory path 和 No Eligible，而不是直接追着 residual spill 做局部重写。

下一步 TODO：

- [x] Phase 36 重构第一批改为“可 review 的结构重构”，允许一次性做更完整的低风险整理，但仍不能改变 kernel 语义：把 SM120 FP8 WS launch/host helper 从 `hstu_fwd_kernel.h` 拆到独立 header，使 BF16 C++ launch helper 与 FP8 WS launch helper 在文件边界上分离。
- [x] 清理 FP8 WS kernel 的死代码和历史 debug 路径：删除当前已经不可达的 non-persistent 分支判断和 FP8 WS 主循环里的 `debug_gemm1_only` 分支，删减重复或过长注释，只保留同步语义、layout invariant 和 ISA/SASS 依赖。
- [x] 对当前函数抽取重构先做锁频全量 benchmark，和 `2benchmark_results/663_gpu2407MHz_phase36_refactor_debug_restore_kernel.log` / `665` 等同频基线比较。若出现系统性 regression，先定位并修复或局部回退；只有确认当前重构可保留后，才进入下一刀代码结构调整。
- [ ] 抽取 FP8 WS load warp 可复用 helper：优先处理 Q/K/V/RAB/O 中共享的 mbarrier parity、stage pointer 选择、ready/empty wait/arrive 和 TMA issue 辅助逻辑。helper 使用 `__device__ __forceinline__`，保持 SASS 稳定。
- [x] 第二刀 load-warp 控制流重构已尝试并拒绝：把 Q/K/V/O warp role 判断移到 persistent schedule 外层会导致 D256 paged complex specialization 新增 `STACK:192` 和大量 `STL/LDL`。当前保留 tile 内 role 判断；后续如再尝试，只能先拆出更小的 per-role helper 或分 kernel/specialization 降低单 kernel codegen 压力，且必须先看 SASS 再 benchmark。
- [ ] 抽取 math warp 长段逻辑：先移动无状态或低状态函数，例如 RAB/mask/epilogue 小 helper；大量捕获局部变量的主循环只在编译和 SASS 稳定后继续拆，目标是让 `hstu_compute_attn_1rowblock_sm120_fp8_ws` 能直接读出 warp-specialized 流程。
- [ ] 每批重构后立即构建，并跑 `hstu_test.py`、`sweep_accuracy.py`、`run_hstu8_examples.sh`。即使是机械拆分，也必须完整跑 correctness，避免模板实例化、include 顺序或 dead branch 删除造成隐性问题。
- [ ] 对重构后的版本 dump FP8 WS 代表 SASS/resource，确认 D128/D256 full、causal、target、RAB、paged RAB 没有新增 `LDL/STL` 或更高 `STACK`。
- [ ] 锁频跑一次 kernel-only benchmark，对比 `2benchmark_results/651_d605d70d_gpu2407MHz_phase36_setmaxnreg_conditional_full.log`。重构阶段的保留条件是不出现系统性性能 regression；没有性能提升也可以保留，只要代码可读性明显改善。
- [ ] 重构完成后再回到性能优化：针对 RAB/paged target 的 memory path 和 No Eligible 做 profile-driven 优化；已拒绝的 target sparse direct-global、target skip-masked-after-mask、D256 paged RAB paired persistent 不再重复尝试，除非输入语义或调度条件发生变化。
