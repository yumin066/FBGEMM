...
#pragma unroll
    ...
    if constexpr (Use_TMA_KV) {
        ...
        // fence.proxy.async.shared::cta: makes mbarrier init visible to async proxy (TMA).
...
#pragma unroll
    ...
    auto fwd_step_fp8bs = [&](int nb, int nb_next, int masking_step) {
      ...
      if constexpr (!Use_TMA_KV) {
          ...
          // Phase 5: TMA → wait on mbarrier using try_wait (yields warp, avoids busy-spin stall).
          // Use try_wait.parity with 0x989680 (~10M cycle) timeout per CUTLASS ClusterTransactionBarrier.
          {
              uint32_t smem_addr = static_cast<uint32_t>(__cvta_generic_to_shared(kv_mbar_ptr));
              uint32_t done = 0;
              do {
...
inline __device__ void hstu_compute_attn_1rowblock_sm120_fp8_ws(
  ...
  constexpr int kNMathThreads = Kernel_traits::kNThreads;  // 256
...
template <typename Kernel_traits, typename TMA_K_t, typename TMA_Vt_t>
__global__ void __launch_bounds__(Kernel_traits::kNThreadsTotal)
...
void run_hstu_fwd_sm120_fp8_ws_impl(Hstu_fwd_params& params, cudaStream_t stream) {
  ...
  // Launch with kNThreadsTotal=288 (8 math warps + 1 load warp).
  kernel<<<grid, Kernel_traits::kNThreadsTotal, smem_size, stream>>>(tma_params);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void run_hstu_fwd_sm120(Hstu_fwd_params& params, cudaStream_t stream) {
  ...
  if constexpr (Is_fp8_type) {
    if constexpr ((kBlockN % 128) == 0) {
      if constexpr (!Has_rab) {
        // TEMP DIAGNOSTIC: Use Phase 5 TMA to verify non-WS path still works
