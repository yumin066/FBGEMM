...
KERNEL_PERSISTENT_D256 = _env_int("HSTU_CUTILE_FP8_PERSISTENT", 0) != 0
Q_LATENCY_D256 = _env_optional_int("HSTU_CUTILE_FP8_Q_LATENCY", 4)
K_LATENCY_D256 = _env_optional_int("HSTU_CUTILE_FP8_K_LATENCY", 5)
V_LATENCY_D256 = _env_optional_int("HSTU_CUTILE_FP8_V_LATENCY", 5)
SCALE_LATENCY_D256 = _env_optional_int("HSTU_CUTILE_FP8_SCALE_LATENCY", 3)
O_LATENCY_D256 = _env_optional_int("HSTU_CUTILE_FP8_O_LATENCY", 2)
...
if ct is not None:
    ...
    def hstu_fp8_d256_persistent_kernel(
        ...
        for linear_tile in range(bid, total_tiles, num_tile_blocks):
            ...
            for n_block in range(n_block_end):
                ...
                if full_tiles:
                    acc_s = acc_s * alpha
                    acc_s = acc_s / (ct.exp(-acc_s) + 1.0)
                    p_fp8 = acc_s.astype(Q.dtype)
                else:
                    ...
                    acc_s = acc_s * alpha
                    acc_s = acc_s / (ct.exp(-acc_s) + 1.0)
                    acc_s = ct.where(valid, acc_s, 0.0)
                    p_fp8 = acc_s.astype(Q.dtype)
                p_scale = ct.full(
    ...
    @ct.kernel(**_KERNEL_HINTS_D256)
    def hstu_fp8_d256_single_tile_kernel(
        Q,
        K,
        V,
        QScale,
    ...
    hstu_fp8_d256_kernel = (
        hstu_fp8_d256_persistent_kernel
        if KERNEL_PERSISTENT_D256
        else hstu_fp8_d256_single_tile_kernel
    )

else:
    hstu_fp8_d256_persistent_kernel = None
    hstu_fp8_d256_single_tile_kernel = None
    hstu_fp8_d256_kernel = None
